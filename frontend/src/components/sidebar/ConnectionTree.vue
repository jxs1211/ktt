...
const onUpdateSelectedKeys = (keys, option) => {
  ...
  if (!isEmpty(keys)) {
    console.log("updated connections keys: ", keys, option[0].label);
...
const saveSort = debounce(connectionStore.saveConnectionSorted, 1500, {
  trailing: true,
});
const handleDrop = ({ node, dragNode, dropPosition }) => {
  ...
  if (
    node.type === ConnectionType.Group &&
    dragNode.type === ConnectionType.Group
  ) {
    return;
  }
