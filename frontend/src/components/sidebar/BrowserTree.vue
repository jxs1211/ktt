...
const handleKeyUp = () => {
  ...
  if (nodeIndex <= 0) {
    if (
      parentNode.type === ConnectionType.RedisKey ||
...
const handleKeyDelete = () => {
  ...
  switch (node.type) {
    case ConnectionType.RedisKey:
      dialogStore.openDeleteKeyDialog(
        props.server,
        db,
        isEmpty(redisKey) ? "*" : redisKey + ":*",
      );
      break;
    case ConnectionType.RedisValue:
      $dialog.warning(
        i18n.t("dialogue.remove_tip", { name: redisKeyName }),
        () => {
          browserStore.deleteKey(props.server, db, redisKey).then((success) => {
            if (success) {
              $message.success(
                i18n.t("dialogue.delete.success", { key: redisKeyName }),
              );
            }
          });
        },
...
const renderPrefix = ({ option }) => {
  switch (option.type) {
    ...
    case ConnectionType.RedisValue:
      ...
      const loading =
        isEmpty(option.redisType) || option.redisType === "loading";
      if (loading) {
...
const calcLayerMenu = (loading) => {
  return [
    ...
    h(IconButton, {
      tTooltip: props.fullLoaded
        ? "interface.reload"
        : "interface.reload_disable",
      icon: Refresh,
      loading: loading === true,
...
const updateKeyDetail = (node) => {
  if (node.type === ConnectionType.RedisValue) {
    ...
    if (
      !isEmpty(preK) &&
      preK === node.key &&
      Date.now() - lastLoadKey.value > 1000
    ) {
  ...
  return {
    ...
    onDblclick: () => {
      ...
      if (!props.checkMode) {
        ...
        nextTick().then(() =>
          tabStore.toggleExpandKey(props.server, option.key),
        );
      }
    },
    onContextmenu(e) {
