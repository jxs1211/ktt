...
import {
  Publish as PublishSend,
  StartSubscribe,
  StopSubscribe,
} from "wailsjs/go/services/pubsubService.js";
...
const columns = computed(() => [
  ...
  },
  {
    ...
    filter: (value, row) => {
      return (
        value === "" ||
        row.client === value.toString() ||
        row.addr === value.toString()
      );
    },
    width: 200,
    align: "center",
...
const onPublish = async () => {
  ...
  setTimeout(() => {
    if (
      publishData.lastShowReceived > 0 &&
      Date.now() - publishData.lastShowReceived > 2000
    ) {
...
<template>
  <div class="content-log content-container fill-height flex-box-v">
    ...
    <div class="total-message">
      {{ $t("pubsub.receive_message", { total: size(data.list) }) }}
    </div>
        ...
        >
          <template #suffix>
            <transition mode="out-in" name="fade">
              <n-tag
                v-show="publishData.lastShowReceived > 0"
                bordered
                size="small"
                type="success"
              >
