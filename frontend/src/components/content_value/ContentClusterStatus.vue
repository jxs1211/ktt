...
const _updateChart = (info) => {
  ...
  timeLabels = slice(
    timeLabels,
    Math.max(0, timeLabels.length - statusHistory),
  );
  ...
  // connected clients
  {
    ...
    dataset = slice(dataset, Math.max(0, dataset.length - statusHistory));
    connectedClients.value = generateData(connectedClients.value, timeLabels, [
      dataset,
    ]);
  }
  ...
  // network input/output rate
  {
    ...
    networkRate.value = generateData(networkRate.value, timeLabels, [
      dataset1,
      dataset2,
    ]);
  }
...
const onToggleRefresh = (on) => {
  if (on) {
    ...
    connectionStore.saveRefreshInterval(
      props.server,
...
const uptime = computed(() => {
  let seconds = parseInt(
    get(serverInfo.value, "Server.uptime_in_seconds", "0"),
  );
  seconds /= 60;
  if (seconds < 60) {
    // minutes
...
watch(
  ...
  () => {
    ...
    connectedClients.value.datasets[0].label = i18n.t(
  ...
  return [
    {
      ...
      title: () => {
        return h(
          NSpace,
          { wrap: false, wrapItem: false, justify: "center" },
          () => [
            h(
              "span",
              { style: { fontWeight: "550", fontSize: "15px" } },
              i18n.t("status.client.title"),
            ),
            h(IconButton, {
              icon: Refresh,
              size: 16,
    ...
    <n-card embedded>
      <template #header>
        <n-space :wrap-item="false" align="center" inline size="small">
          {{ props.server }}
          <n-tooltip v-if="clusterVersion">
            Cluster Version
            <template #trigger>
              <n-tag size="small" type="primary">{{ clusterVersion }}</n-tag>
            </template>
          </n-tooltip>
        </n-space>
      </template>
              ...
              <template #icon>
                <n-icon :size="props.size">
                  <refresh
                    ...
                    :color="
                      pageState.autoRefresh ? themeVars.primaryColor : undefined
                    "
      ...
      <n-grid style="min-width: 500px" x-gap="5">
        <n-gi :span="6">
          <n-statistic :label="$t('status.status')" :value="clusterStatus">
            <!-- <template #suffix>{{ $t(uptime.unit) }}</template> -->
          </n-statistic>
        </n-gi>
        <n-gi :span="6">
          <n-statistic
            :label="$t('status.catched_errors')"
            :value="errorsCount"
          >
            <!-- <template #suffix>
              <n-tooltip
      ...
      >
        <template #suffix>
          <div v-if="tabVal === 'info'" style="padding-right: 10px">
            <n-input
              v-model:value="infoFilter.keyword"
              clearable
              placeholder=""
            >
              <template #prefix>
