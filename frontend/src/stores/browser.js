...
const useBrowserStore = defineStore("browser", {
  ...
  actions: {
    ...
    async loadAllKeys(server, db) {
      const { match, type: keyType, exact } = this.getKeyFilter(server);
      const { keys, maxKeys } = await this._loadKeys({
        server,
        db,
        match,
        exact,
    ...
    getParentNode(key) {
      ...
      return serverInst.nodeMap.get(
        `${ConnectionType.RedisKey}/${join(parentKey, separator)}`,
      );
    },
    ...
    async addHashField({ server, db, key, action, fieldItems, reload }) {
      try {
        const { data, success, msg } = await AddHashField(
          server,
          db,
          key,
          action,
    ...
    async getHashField({
      ...
      try {
        const { data, success, msg } = await GetHashValue({
          server,
          db,
          key,
          field,
    ...
    async prependListItem({ server, db, key, values, reload }) {
      try {
        const { data, success, msg } = await AddListItem(
          server,
          db,
          key,
          0,
    ...
    async addSetItem({ server, db, key, value, reload }) {
      try {
        ...
        const { data, success, msg } = await SetSetItem(
          server,
          db,
          key,
          false,
    ...
    async removeSetItem({ server, db, key, value, reload }) {
      try {
        const { data, success, msg } = await SetSetItem(server, db, key, true, [
          value,
        ]);
        if (success) {
          const { removed } = data;
    ...
    async addZSetItem({ server, db, key, action, vs, reload }) {
      try {
        const { data, success, msg } = await AddZSetValue(
          server,
          db,
          key,
          action,
    ...
    async addStreamValue({ server, db, key, id, values, reload }) {
      try {
        const {
          ...
          success,
          msg,
        } = await AddStreamValue(server, db, key, id, values);
        if (success) {
          const { added = [] } = data;
          if (!isEmpty(added)) {
            const tab = useTabStore();
    ...
    async removeStreamValues({ server, db, key, ids, reload }) {
      ...
      try {
        const {
          ...
          success,
          msg,
        } = await RemoveStreamValues(server, db, key, ids);
        if (success) {
          const tab = useTabStore();
          tab.removeValueEntries({
            server,
    ...
    async setTTLs(server, db, keys, ttl) {
      ...
      try {
        ...
        // }
        const { data, success, msg } = await BatchSetTTL(
          server,
          db,
          keys,
          ttl,
    ...
    async deleteKeys(server, db, keys) {
      const msgRef = $message.loading(
        i18nGlobal.t("dialogue.delete.deleting"),
      ...
      try {
        const { success, msg, data } = await DeleteKeys(
          server,
          db,
          keys,
          serialNo,
      ...
      if (canceled) {
        ...
        // no fail
        $message.success(
          i18nGlobal.t("dialogue.delete.completed", {
    ...
    async deleteByPattern(server, db, pattern) {
      ...
      let canceled = false;
      try {
        const { success, msg, data } = await DeleteKeysByPattern(
          server,
          db,
          pattern,
        );
    ...
    async exportKeys(server, db, keys, path, expire) {
      ...
      try {
        const { data, success, msg } = await ExportKey(
          server,
          db,
          keys,
          path,
    ...
    async importKeysFromCSVFile(server, db, path, conflict, ttl, reload) {
      ...
      try {
        const { data, success, msg } = await ImportCSV(
          server,
          db,
          path,
          conflict,
      ...
      if (canceled) {
        ...
        // finish
        $message.success(
          i18nGlobal.t("dialogue.import.import_completed", {
            success: imported,
            ignored,
          }),
        );
    ...
    async renameKey(server, db, key, newKey) {
      ...
      if (success) {
        ...
        return {
          success: true,
          nodeKey: `${server}/db${db}#${ConnectionType.RedisValue}/${newKey}`,
        };
      } else {
        return { success: false, msg };
      }
    ...
    async getCmdHistory(pageNo, pageSize) {
      ...
      try {
        const { success, data = { list: [] } } = await GetCmdHistory(
          pageNo,
          pageSize,
        );
        const { list } = data;
